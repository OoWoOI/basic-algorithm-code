...
Node *insert_maintain(Node *root, Node *child, int pos) {
    ...
    int spos = MAX_M / 2;//new root index in root next arr
    Node *node1 = getNewNode();//pre part : 0 - (spos - 1)
    Node *node2 = getNewNode();//last part: (spos + 1) - n
    node1->n = spos;
